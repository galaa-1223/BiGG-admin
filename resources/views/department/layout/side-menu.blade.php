@extends('../department/layout/main')

@section('head')
    <title>{{ $page_title.' :: '.config('app.department_title') }}</title>
@endsection

@section('content')
    @include('../department/layout/components/mobile-menu')
    <div class="flex">
        <!-- BEGIN: Side Menu -->
        <nav class="side-nav">
            <a href="" class="intro-x flex items-center pl-5 pt-4">
                <img alt="Midone Tailwind HTML department Template" class="w-6" src="{{ asset('dist/images/logo.svg') }}">
            ...
            <ul>
                @foreach (config('app.department_side_menu') as $menu)
                    @if ($menu == 'devider')
        ...
        <!-- BEGIN: Content -->
        <div class="content">
            @include('../department/layout/components/top-bar')
            @yield('subcontent')
        </div>
        <!-- END: Content -->
    </div>
